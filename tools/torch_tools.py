...
def augment(paths, texts, vids, num_items=4, target_length=1024):
    mixed_sounds, mixed_captions, mixed_vids = [], [], []
    ...
    for (i, j) in selected_combinations:
        ...
        mixed_vids.append(vids[i])
        
    waveform = torch.tensor(np.concatenate(mixed_sounds, 0))
    waveform = waveform / torch.max(torch.abs(waveform))
    waveform = 0.5 * waveform
    
    return waveform, mixed_captions, mixed_vids


def augment_wav_to_fbank(paths, texts, vids, num_items=4, target_length=1024, fn_STFT=None):
    assert fn_STFT is not None
    
    waveform, captions, vids = augment(paths, texts, vids, num_items=num_items, target_length=target_length)
    fbank, log_magnitudes_stft, energy = get_mel_from_wav(waveform, fn_STFT)
    fbank = fbank.transpose(1, 2)
    log_magnitudes_stft = log_magnitudes_stft.transpose(1, 2)
    ...
    return fbank, log_magnitudes_stft, waveform, captions, vids